...
def MACRO(G, macro_mapping, macro_types):
    """
    ...
    n_macros_spatem2 = len([i for i,j in macro_types.items() if j=='spatem2'])
    ...
    for final_node_i in nodes_in_macro_network:
        ...
        if final_node_i_type == 'spatial':
            ...
            # indices of the FINAL macro network that arent in the current macro
            # note: "nodes_in_macro_network" is of len=size of final macro, and
            # its elements correspond to the *values* in macro_mapping
            ...
            nodes_outside_macro_mic_index = [i for i,j in macro_mapping.items()\
            ...
            if sum(macro_row_sum)==0: # then there are no inputs from outside
        ...
        if final_node_i_type == 'spatem1':
            # these are the indices of the micro nodes inside this spatem1 macro
            micros_in_macro_i = [i for i,m in macro_mapping.items() \
                                      if m == final_node_i]
            ...
            # same list as above but this one uses the microscale indices as its
            # elements so that you can get the right exit rates from Wout_micro
            nodes_outside_macro_mic_index = [i for i,j in macro_mapping.items()\
                                                  if j in nodes_outside_macro_i]

            # this is a len(nodes_outside_macro_mic_index)x1 vector of exitrates
            Wout_macro_i_exitrates = sum(Wout_macro_subgraph_weighted[:,\
                                         nodes_outside_macro_mic_index])
            
            # this is the total exit rate probability from the macro in question
            Wout_macro_i_exitrates_sum = sum(Wout_macro_i_exitrates)
            
            # if there are no edges with weights outside the macro, add selfloop
            ...
            # calculate the out-weights of the nodes that make up the macro
                ...
                for i, wij_ind in enumerate(new_indices):
                    ...
                    W_i_out_final[wij_ind] += Wout_macro_i_exitrates_norm[old_i]
        ...
        if final_node_i_type == 'spatem2':
            ...
            # W_i_out_final node will just have a single 1.0 edge weight to the 
            # mu_mu macro node. This bit is primarily for finding the 
            # edge weights from mu_mu.
            ...
            if Wout_macro_i_exitrates_sum == 0:
                ...
                for i, wij_ind in enumerate(new_indices):
                    old_i = out_indices[i]
                    wij_ind = macro_mapping[wij_ind] 
                    W_mu_out_final[wij_ind] +=Wout_macro_i_exitrates_norm[old_i]
...
def SELECT_MACRO(G_micro, node_i_macro, possible_mapping, macro_types, F=True): 
    ...
    nodes_in_macro_with_outside_output = [i for i,j in \
                      edges_from_micro_in_macro.items() if \
                      len(set(rest_of_nodes).intersection(set(list(j))))>0]
    nodes_in_macro_with_outside_input = [i for i,j in \
                      edges_to_micro_in_macro.items() if \
                      len(set(rest_of_nodes).intersection(set(list(j))))>0]
    ...
    if len(cond) == 0: 
        G_macro = MACRO(G_micro, possible_mapping, macro_types_spatem2)
        return G_macro, macro_types_spatem2    
    ############################################################################

    ########################### For spatiotemporal1 ############################
    else:
        G_macro = MACRO(G_micro, possible_mapping, macro_types_spatem1)
...
def CAUSAL_EMERGENCE(G, span=-1, thresh=1e-4, t=500, types=False, printt=True):
    ...
    CE (dict): a dictionary with the following information
      ...
      - macro_types (dict): the dictionary associated with each type of 
                            ...
                            set to 'spatem1'.
      - inaccuracy (np.ndarray): a sequence of inaccuracy values associated with
                                 the successful macro_mapping.
    ...
    macro_mapping    = dict(zip(micro_nodes_left, 
                                micro_nodes_left))
    ...
    for node_i in micro_nodes_left:
        ...
        while len(queue) > 0:
            ...
            if types:
                G_macro, macro_types_tmp = SELECT_MACRO(G_micro, node_i_macro,\
                                                possible_mapping, macro_types)
            else:
                macro_types_tmp = macro_types.copy()
                macro_types_tmp[node_i_macro] = "spatem1"
                G_macro = MACRO(G_micro, possible_mapping, macro_types_tmp)
            ...
            if EI_macro - EI_current > thresh:
                ...
                # inaccuracy check here. make sure this is correct, especially
                # with the "continue" call below
                # inaccuracy_i = INACCURACY(G_micro, G_macro, \
                                          # possible_mapping,macro_types_tmp,t)
                # if sum(inaccuracy_i['inaccuracies'][-10:]) > thresh:
                    # inaccurate_macro_pairs += 1
                    # continue
    ...
    CE = {}
    G_macro  = MACRO(G_micro, macro_mapping, macro_types)
    G_macro  = CHECK_NETWORK(G_macro)
    EI_macro = EI(G_macro)
    if macro_types == {}:
        EI_macro = EI_micro
    ...
    inaccuracies = INACCURACY(G_micro, G_macro, macro_mapping, macro_types, t)
    CE['G_macro']       = G_macro
    CE['G_micro']       = G_micro
...
# def CAUSAL_EMERGENCE_s(G, span=-1, frac=0.1, thresh=1e-4, 
#                        t=500, types=True, printt=True):
#     """
#     Given a microscale network, $G$, this function iteratively checks different
#     coarse-grainings to see if it finds one with higher effective information. 
...
#             G_macro  = CHECK_NETWORK(G_macro)
#             EI_macro = ESTIMATE_EI(G_macro, frac)
# #             EI_macro = EI(G_macro)
#             if np.isinf(EI_macro):
#                 return G_macro
...
def INACCURACY(G_micro, G_macro, macro_mapping, macro_types, t=500):
    """
    ...
    macro mapping is the KL Divergence of the expected location and distribution
    ...
    distribution_over_macro = np.zeros(len(nodes_in_macro_network) +\
                                            spatem2_count)
    for y in range(len(remaining_micro_nodes)):
    ...
    for ti in range(1, t):
        Wout_micro_t = Wout_micro_t.dot(Wout_micro) # transitions over one step
    ...
    for ti in range(1, t):
        Wout_macro_t = Wout_macro_t.dot(Wout_macro) # transitions over one step
...
def ALL_POSSIBLE_MAPPINGS(G):
    ...
    def algorithm_u(ns, m):
        ...
        partitions with a given number of blocks. Since Knuth's algorithm is not
...
def INTERVENTION_DISTRIBUTION(G,macro_mapping,scale='macro', conditional=True):
    """
...
def REORDER_ELEMENTS(Win_micro, macro_mapping):
    ...
    overlap = list(set(nodes_in_micro_network).intersection(\
...
# def INACCURACY_OLD(G, macro_mapping, conditional=True, t=10):
#     """
#     The inaccuracy associated with a macro mapping is the KL Divergence
#     of the expected location and distribution of random walkers following
#     the same intervention distribution on the micro and macro representation
#     of the network. 
...
#     -------
#     inaccuracy_dict (dict): dictionary with tensors of TPMs for micro_out, 
#                             macro_out, micro_in, micro_out, and a list of 
#                             inaccuracies.
#     """    
...
#     G = CHECK_NETWORK(G)
#     Wout_micro = W_OUT(G)
#     Wout_macro = MACRO(G, macro_mapping, macro_types)
    
#     # the intervention distribution at the microscale, given the macroscale
#     IntD_micro = INTERVENTION_DISTRIBUTION(Wout_micro, macro_mapping, 
#                                            'micro', conditional)
#     IntD_macro = INTERVENTION_DISTRIBUTION(Wout_macro, macro_mapping, 
#                                            'macro', conditional)
    
#     # if the micro = macro, the IntDs are both max entropy
#     if sum([k==v for k, v in macro_mapping.items()]) == G.number_of_nodes():
#         IntD_micro = np.ones(G.number_of_nodes()) / G.number_of_nodes()
#         IntD_macro = np.ones(G.number_of_nodes()) / G.number_of_nodes()

#     inaccuracy_dict['InterventionDist_micro'] = IntD_micro
#     inaccuracy_dict['InterventionDist_macro'] = IntD_macro
    
#     Win_micro = W_IN(Wout_micro, IntD_micro)
#     Win_macro = W_IN(Wout_macro, IntD_macro)
...
#     # at t = ti steps in the future. 
#     list_of_micros = [Wout_micro]
#     Wout_micro_t = Wout_micro
#     for ti in range(1, t):
#         Wout_micro_t = Wout_micro_t.dot(Wout_micro)
#         list_of_micros.append(Wout_micro_t)

#     # pre-populate a list of transition probability matrices for the macro
#     # at t = ti steps in the future. 
#     list_of_macros = [Wout_macro]
#     Wout_macro_t = Wout_macro
#     for ti in range(1, t):
#         Wout_macro_t = Wout_macro_t.dot(Wout_macro)
#         list_of_macros.append(Wout_macro_t)

#     inaccuracy_dict['Wout_micro_list'] = list_of_micros
#     inaccuracy_dict['Wout_macro_list'] = list_of_macros    
    
#     list_of_EffectDist_micro = []
#     for micro in list_of_micros:
#         Win_j = micro.T.dot(IntD_micro)
#         ED_micro = Win_j / sum(Win_j)
#         EffectDist_micro = REORDER_ELEMENTS(np.array(ED_micro), macro_mapping)
#         list_of_EffectDist_micro.append(EffectDist_micro)

#     list_of_EffectDist_macro = []
#     for macro in list_of_macros:
#         Win_j = macro.T.dot(IntD_macro)
#         ED_macro = Win_j / sum(Win_j)
#         EffectDist_macro = ED_macro
#         list_of_EffectDist_macro.append(EffectDist_macro)
        
#     inaccuracy_dict['EffectDist_micro'] = list_of_EffectDist_micro
#     inaccuracy_dict['EffectDist_macro'] = list_of_EffectDist_macro    
    
#     inaccuracies = [sp.stats.entropy(list_of_EffectDist_macro[i], 
#                                      list_of_EffectDist_micro[i], base=2) 
#                     for i in range(len(list_of_EffectDist_macro))]
...
# def MACRO_OLD(G, macro_mapping, conditional=True):
#     """
#     Coarse-grains a network according to the specified macro_mapping. If 
#     conditional = True, this function will create a coarse-graining with
#     conditional macro nodes.
...
#     if not conditional:
#         # get new rows of the empty macro transition probability matrix
#         for macronode_i in micro_to_macro_list:
#             macro_row_list.append(sum(Wout_micro[macronode_i,:]))

#     else: # conditional macro-nodes
#         # get new rows of the empty macro transition probability matrix
#         for macronode_i in micro_to_macro_list:
#             macro_row_sum = np.zeros(Wout_micro.shape[0])

#             # get the rows of the micro TPM that correspond to macro
#             macro_rows_temp = Wout_micro[macronode_i,:] 
#             input_probs_to_macro = Wout_micro.T[macronode_i].T
        
#             # get the input to the nodes in the macro 
#             # note: if you want to condition over a number of the 
#             # set of nodes, specify that here. 
#             for i in range(len(macro_row_sum)):
#                 # if i in specfied_conditional_list: 
#                     # macro_row_sum += macro_rows_temp.T.dot(\
#                     # input_probs_to_macro[i].T)
#                 macro_row_sum +=macro_rows_temp.T.dot(input_probs_to_macro[i].T)
        
#             # stack the macro rows into the list
#             macro_row_list.append(macro_row_sum)
            
#     macro_rows = np.vstack(macro_row_list)

#     # get new cols of the empty macro transition probability matrix
#     macro_col_list = []
#     for macronode_i in micro_to_macro_list:
#         macro_col_list.append(sum(Wout_micro.T[macronode_i,:]))
#     macro_cols = np.vstack(macro_col_list)    
#     macro_cols = macro_cols.T

#     # get diagonal square in the bottom right (macro-macro connections)
#     macro_bottom_right = np.zeros((N_macronodes, N_macronodes))
#     for macro_i in range(N_macronodes):
#         for macro_j in range(N_macronodes):
#             X = sum(macro_row_list[macro_i][micro_to_macro_list[macro_j]])
#             macro_bottom_right[macro_i, macro_j] = X

#     # put them all together in a matrix that is too big
#     too_big_macro = np.block([[Wout_micro, macro_cols], 
#                               [macro_rows, macro_bottom_right]])

#     macro_out = too_big_macro[nodes_in_macro_network,:] \
#                              [:,nodes_in_macro_network]

#     rowsums = macro_out.sum(axis=1)[:, np.newaxis]
#     nodes_with_output = np.nonzero(rowsums)[0]
#     macro_out[nodes_with_output] = macro_out[nodes_with_output] / \
#                                      rowsums[nodes_with_output]
#     M = macro_out

#     return M


# def CAUSAL_EMERGENCE(G, span=-1, thresh=1e-4, t=500, types=True, printt=True):
#     """
#     Given a microscale network, $G$, this function iteratively checks different
#     coarse-grainings to see if it finds one with higher effective information. 
...
#       - G_macro (nx.Graph): a coarse-grained description of the micro network.
#       - G_micro (nx.Graph): the original microscale network.
#       - macro_mapping (dict): the mapping that most-successfully increased the 
#                               effective information of the network.
#       - inaccuracies (np.ndarray): a sequence of inaccuracy values associated
